# 👋 Hi, I'm Kalyan

### 🚀 AI | Gen AI Engineer | Python | FastAPI | LangChain | Google Cloud

I'm a **Gen AI Engineer** with **4+ years of experience** building scalable AI-powered applications using Large Language Models (LLMs), Retrieval-Augmented Generation (RAG), and cloud-native architectures.

I specialize in developing intelligent systems using **Python** **LangChain**, **Google ADK**, **Vertex AI**, **FastAPI**, and **Google Cloud Platform** for telecom and industrial automation solutions.

---

## 💼 About Me

- 🤖 Building GenAI & LLM-powered applications
- 🧠 Experienced in RAG, Multi-Agent Systems & Prompt Engineering
- ☁️ Google Cloud & Azure Developer
- ⚡ FastAPI Backend Development
- 📚 Passionate about AI Automation & Cloud Technologies

---

## 🌐 Connect with Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-blue?style=for-the-badge&logo=linkedin)](https://linkedin.com/in/kalyan-chary-katakam-b6550b198)
[![Gmail](https://img.shields.io/badge/Gmail-red?style=for-the-badge&logo=gmail)](mailto:kalyanchary140@gmail.com)
[![Portfolio](https://img.shields.io/badge/Portfolio-black?style=for-the-badge&logo=google-chrome)](https://yourportfolio.com)

---

# 🛠 Tech Stack

### 🤖 Generative AI

![LangChain](https://img.shields.io/badge/LangChain-121212?style=for-the-badge)
![LangGraph](https://img.shields.io/badge/LangGraph-blue?style=for-the-badge)
![Google ADK](https://img.shields.io/badge/Google_ADK-4285F4?style=for-the-badge)
![Vertex AI](https://img.shields.io/badge/Vertex_AI-4285F4?style=for-the-badge)
![RAG](https://img.shields.io/badge/RAG-green?style=for-the-badge)
![LLMs](https://img.shields.io/badge/LLMs-orange?style=for-the-badge)
![Prompt Engineering](https://img.shields.io/badge/Prompt_Engineering-red?style=for-the-badge)
![HuggingFace](https://img.shields.io/badge/HuggingFace-yellow?style=for-the-badge&logo=huggingface)

### 💻 Languages

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge)

### ⚙ Backend

![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi)
![Flask](https://img.shields.io/badge/Flask-black?style=for-the-badge&logo=flask)

### ☁ Cloud

![Google Cloud](https://img.shields.io/badge/Google_Cloud-4285F4?style=for-the-badge&logo=googlecloud)
![Azure](https://img.shields.io/badge/Azure-0078D4?style=for-the-badge&logo=microsoftazure)
![Cloud Run](https://img.shields.io/badge/Cloud_Run-4285F4?style=for-the-badge)

### 🗄 Database

![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=for-the-badge&logo=redis)
![BigQuery](https://img.shields.io/badge/BigQuery-4285F4?style=for-the-badge)

### ⚙ DevOps & Tools

![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git)
![MQTT](https://img.shields.io/badge/MQTT-660066?style=for-the-badge)
![WebSockets](https://img.shields.io/badge/WebSockets-blue?style=for-the-badge)

---

# 📌 Featured Projects

### 🤖 Multi-Agent Telecom Assistant
AI-powered telecom support platform using Google ADK, LangChain and FastAPI.

### 📄 Document QA System
RAG-powered document question answering using Vertex AI Vector Search and BigQuery.

### 📑 AI RFP Generator
Automated proposal generation using Gemini, LangChain and Document AI.

### 🏭 Industrial IoT Platform
Real-time factory monitoring using MQTT, Redis and Python.

---

### 💡 Quote

> *"🚀Building intelligent systems that solve real-world problems with AI."*
